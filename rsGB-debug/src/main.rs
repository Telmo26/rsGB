...
fn main() {
    ...
    let mut debug_buffer:[u32; 32768]  = [0x00FFFFFF; (16 * 8) * (32 * 8)];
    ...
    while emu_window.is_open() && !emu_window.is_key_down(Key::Escape) {
        ...
        update_debug_window(&mut debug_window, &mut debug_buffer);
    }
...
fn update_debug_window(debug_window: &mut Window, buffer: &mut [u32]) {
    let (x_draw, y_draw) = (0, 0);
    let tile_num = 0;
