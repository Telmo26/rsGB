...
impl Instruction {
    ...
    pub fn to_str(&self, cpu: &CPU) -> String {
        ...
        match self.mode {
            ...
            R_D8 => format!("{:?} {:?}, {:02X}", self.in_type, self.reg_1, cpu.fetched_data),
            R_A8 => format!("{:?} {:?}, ({:04X})", self.in_type, self.reg_1, cpu.fetched_data),
            R_MR => format!("{:?} {:?}, ({:?})", self.in_type, self.reg_1, self.reg_2),
            R_HLI => format!("{:?} {:?}, ({:?}+)", self.in_type, self.reg_1, self.reg_2),
