...
fn proc_nop(_cpu: &mut CPU, _dev: &mut impl Peripherals) {}

fn proc_ld(cpu: &mut CPU, dev: &mut impl Peripherals) {
...
fn proc_ldh(cpu: &mut CPU, dev: &mut impl Peripherals) {
...
fn goto_addr(cpu: &mut CPU, dev: &mut impl Peripherals, address: u16, push_pc: bool) {
    if cpu.check_cond() {
...
fn proc_reti(cpu: &mut CPU, dev: &mut impl Peripherals) {
    cpu.int_master_enabled = true;
    proc_ret(cpu, dev);
}

fn proc_di(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_dec(cpu: &mut CPU, dev: &mut impl Peripherals) {
    let mut val = cpu.fetched_data;
...
fn proc_add(cpu: &mut CPU, dev: &mut impl Peripherals) {
    let mut value:u16; 
...
fn proc_adc(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_sub(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_sbc(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    let a = cpu.registers.a;
...
fn proc_and(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    cpu.registers.a &= cpu.fetched_data as u8;
    cpu.set_flags((cpu.registers.a == 0) as u8, 0, 1, 0);
}

fn proc_xor(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    cpu.registers.a ^= cpu.fetched_data as u8;
    cpu.set_flags((cpu.registers.a == 0) as u8, 0, 0, 0);
}

fn proc_or(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    cpu.registers.a |= (cpu.fetched_data & 0xFF) as u8;
    cpu.set_flags((cpu.registers.a == 0) as u8, 0, 0, 0);
}

fn proc_cp(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_cb(cpu: &mut CPU, dev: &mut impl Peripherals) {
    ...
    match bit_op {
        ...
        2 => { // RES
            reg_val &= !(1 << bit);
            cpu.registers.set_reg8(dev, register, reg_val);
        },
        3 => { // SET
            reg_val |= 1 << bit;
            cpu.registers.set_reg8(dev, register, reg_val);
        ...
        0 => { // OTHER
            ...
            match bit {
                0 => { // RLC
                    ...
                    cpu.registers.set_reg8(dev, register, value);
                ...
                5 => { // SRA,
                    ...
                    cpu.registers.set_reg8(dev, register, u);
...
fn proc_rlca(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_rrca(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_rla(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_rra(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_stop(_cpu: &mut CPU, _dev: &mut impl Peripherals) {
    println!("STOP instruction received!")
}

fn proc_daa(cpu: &mut CPU, _dev: &mut impl Peripherals) {
...
fn proc_cpl(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    cpu.registers.a = !cpu.registers.a;
    cpu.set_flags(BIT_IGNORE, 1, 1, BIT_IGNORE);
}

fn proc_scf(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    cpu.set_flags(BIT_IGNORE, 0, 0, 1);
}

fn proc_ccf(cpu: &mut CPU, _dev: &mut impl Peripherals) {
    let cflag = !cpu.c_flag() as u8;
    cpu.set_flags(BIT_IGNORE, 0, 0, cflag);
}

fn proc_halt(cpu: &mut CPU, dev: &mut impl Peripherals) {
    let ie = dev.ie_register();
...
fn proc_ei(cpu: &mut CPU, _dev: &mut impl Peripherals) {
